...
[ 'numhistoria'  => $request->numhistoria ,'nroconsulta'  => $request->nroconsulta ,'fecha'  => $request->fecha ,'enfermedadactual'  => $request->enfermedadactual ,'peso'  => $request->peso ,'talla'  => $request->talla ,'fc'  => $request->fc ,'pp'  => $request->pp ,'circcefalica'  => $request->circcefalica ,'circtoraxica'  => $request->circtoraxica ,'circabdominal'  => $request->circabdominal ,'tasentado'  => $request->tasentado ,'taacostado'  => $request->taacostado ,'tapie'  => $request->tapie ,'resultadoexamencomp'  => $request->resultadoexamencomp ,'eliminado'  => $request->eliminado ,'faringe'  => $request->faringe ,'nariz'  => $request->nariz ,'oido'  => $request->oido ,'laringe'  => $request->laringe ,'cuello'  => $request->cuello ,'otros'  => $request->otros ,'evolucion'  => $request->evolucion ,'observaciones'  => $request->observaciones ,'medico'  => $request->medico ,'sms'  => $request->sms  ] 
         );
         return response()->json([
             'status' => true,
             'registros' => $registros
